...
for_each = var.myvms
  name         = "${lower(each.key)}-${lower(var.company)}-${lower(var.app_name)}-${var.environment}"
...
resource "google_compute_target_pool" "lb_test" {
  ...
  instances        = ["us-east1-b/vm1-bsierad-iac-windows-uml-dev", "us-east1-c/vm2-bsierad-iac-windows-uml-dev", "us-east1-d/vm3-bsierad-iac-windows-uml-dev"]
  name             = "lb-test"
  project          = "project-gcp-uml"
  region           = "us-east1"
